...
target_geometric_elevation_list = []
globedelta_list = []
riserunalt_list = []
celtheoerror_list = []
FEerror_list = []
...
globeabsoluteerror_list = []
FEabsoluteerror_list = []
FEheighterror_list = []
winner_list = []
for index, row in df.iterrows():
    if row['Target']=='Weather Balloon':
        ...
        target_geometric_elevation_list.append(target_geom)
        staralt = float(row['Apparent Alt'])
        globedelta = abs(target_geom-staralt)
        #For this version, we will use the geometric angle, not the refracted angle - "staralt" here is the observed angle of the balloon which we will subtract from the expected geometric angle
        globeresidual = (target_geom-staralt)
        ...
        FEabsoluteerror_list.append(celtheodelta)
        ...
        if celtheodelta>globedelta:
            print(row['Target'],globeobserver.date,'Surface Distance: ',surface_distance_km,' rise/run error:',celtheodelta,' globe geometric error:',globeresidual,' Globe Wins!')
            winner_list.append('Globe Wins!')
        else:
            print(row['Target'],globeobserver.date,'Surface Distance: ',surface_distance_km,' rise/run error:',celtheodelta,' globe geometric error:',globeresidual,' Cel Theo Wins!')
        ...
        if lowestsep > 0.1:
            ...
            starrefraction_list.append(star_uplift)
            globeangleprediction_list.append(globepredictedstargeometricangle)
            #This version uses the geometric angle
            target_geometric_elevation_list.append(target_geom)
            globedelta = abs(target_geom-staralt)
            globeerrors.append(globedelta)
            #Again, use the target's geometric angle compared to the geometric angle of the star to get the residual in this version
            globeresidual = (staralt-target_geom)
            print(globeresidual)
            globedelta_list.append(globeresidual)
            targetaltaboveobserver = row['Target Alt']-row['Observer Alt']
            targetaltoverobserver_list.append(targetaltaboveobserver)
            ...
            while globeresidual > 0.00028:
                ...
                globeresidual = (staralt3-staralt)
            ...
            if celtheodelta>globedelta:
                print(row['Target'],globeobserver.date,'Surface Distance: ',surface_distance_km,' cel theo error:',celtheodelta,' globe geometric error:',globedelta,' Globe Wins!')
                winner_list.append('Globe Wins!')
            else:
                print(row['Target'],globeobserver.date,'Surface Distance: ',surface_distance_km,' cel theo error:',celtheodelta,' globe geometric error:',globedelta,' Cel Theo Wins!')
...
df_out['Target Geometric Elevation Globe Prediction'] = target_geometric_elevation_list
df_out['Star Refraction'] = starrefraction_list
df_out['Star Alt With Refraction Subtracted'] = globeangleprediction_list
df_out['Globe Geometric Error (Degrees)'] = globedelta_list
df_out['Globe Geometric Absolute Error (Degrees)'] = globeerrors
df_out['Celestial Theodolite Error (Degrees)'] = celtheoerror_list
df_out['Flat Earth Rise/Run Error (Degrees)'] = FEerror_list
df_out['Flat Earth Absolute Error (Degrees)'] = FEabsoluteerror_list
df_out['Globe Geometric Occultation Time Error (Seconds)'] = globetimedelta_list
df_out['Celestial Theodolite Occultation Time Error (Seconds)'] = celtheotimedelta_list
df_out['Globe Target Geometric Height Error (meters)'] = globeheighterror_list
df_out['Flat Earth Target Height Error (meters)'] = FEheighterror_list
df_out['Winner'] = winner_list
df_out = df_out.fillna('')
df_out.to_csv('CelestialTheodoliteGeometricResults.csv', sep=',', encoding='utf-8', index=False, header=True)
